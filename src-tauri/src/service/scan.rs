...
use crate::proxy;
use crate::types::{ScanResultPayload, ScanProgressPayload, ScanPortOpenPayload};
use crate::AppState;
...
pub fn start_proxy_scan(
    ...
    let start_ip = Ipv4Addr::from_str(&network)
        .map_err(|_| "无效的网络段格式 (仅支持 IPv4)".to_string())?;
    let mask_ip = Ipv4Addr::from_str(&mask)
        .map_err(|_| "无效的子网掩码格式".to_string())?;
    ...
    tokio::spawn(async move {
        let (tx, mut rx) = tokio::sync::mpsc::channel::<(IpAddr, u16)>(4096);
        ...
        let _p1_found = found_count.clone();
        ...
        let p1_handle = tokio::spawn(async move {
            ...
            for ip_u32 in (network_id + 1)..broadcast_id {
                if p1_stop.load(Ordering::SeqCst) { break; }
                ...
                for port in start_port..=end_port {
                    ...
                    let task = tokio::spawn(async move {
                        if stop.load(Ordering::SeqCst) { drop(permit); return; }
                        ...
                        if proxy::syn_probe(IpAddr::V4(ip), port, syn_timeout_ms).await {
                            ...
                            let _ = tx.send((IpAddr::V4(ip), port)).await;
                        }
        ...
        let p2_handle = tokio::spawn(async move {
            ...
            while let Some((ip, port)) = rx.recv().await {
                if p2_stop.load(Ordering::SeqCst) { break; }
                ...
                let task = tokio::spawn(async move {
                    ...
                    if let Some((protocol, latency_ms)) = proxy::check_proxy(ip, port, verify_timeout_ms).await {
        ...
        let _ = app_handle.emit("scan-progress", final_progress);
